...
| 变量名 | 值 | 说明 |
|--------|-----|------|
| `DEFAULT_API_KEY` | `sk-xxxxxxxx` | 你的 LLM API Key |
| `DEFAULT_MODEL_NAME` | `gpt-4o-mini` | 模型名称 |

如果你用的是 **OpenAI 兼容的第三方服务**（如 DeepSeek、通义千问、自建 vLLM），还需额外配置：

| 变量名 | 值 | 说明 |
|--------|-----|------|
| `DEFAULT_BASE_URL` | `https://api.deepseek.com` | 第三方服务的 API 地址 |
...
| 变量名 | 说明 |
|--------|------|
| `EMAIL_API_KEY` | [resend.com](https://resend.com) API Key |
| `EMAIL_FROM` | 发件地址，如 `bot@yourdomain.com` |
| `ADMIN_EMAIL` | 收件地址（管理员邮箱） |
